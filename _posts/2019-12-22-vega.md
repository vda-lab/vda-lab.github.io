...
### Scale inversions
Looking at the plot that we created with the code above, we see that the plot indeed includes a piece of text that shows the horizontal position of the mouse on the screen (in this case: 1050). This is the mouse position in _screen space_, not in _data space_. Remember that scales made it possible to easily scale numbers from the "domain" to a "range".

<!--

Here we basically need the opposite: we need to go from the range back to the original domain. Enter scale inversions.

If we alter the event above to

{% highlight json %}
{"events":"mousemove", "update": "invert('xscale', event.x)"}
{% endhighlight %}

we take `event.x` and apply an inverted `xscale` scale on it. Try this out. Unfortunately, the value is still wrong as :

<img src="{{ site.baseurl }}/assets/vega-scaleinversion-wrong.png" /> -->
